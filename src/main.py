...
class PlacesIngestionPipeline:
    ...
    def __init__(
        ...
        # Используем настройки из конфигурации для обогатителя
    ...
    def save_store_sync(self, store_data: Dict) -> Optional[Store]:
        ...
        try:
            with db_manager.get_session() as session:
                # Проверяем существование записи (SQLAlchemy 2.0 стиль)
                stmt = select(Store).where(Store.place_id == place_id)
                existing_store = session.scalar(stmt)
